...
local function highlight_as(hl_group, buf_nr, row_nr, start, stop)
	start = start or 0
	stop = stop or -1
	vim.api.nvim_buf_add_highlight(buf_nr, draft_ns, hl_group, row_nr, start, stop)
end
...
local function make_indent(buf_nr, row_nr, length)
	...
	vim.api.nvim_buf_set_extmark(buf_nr, draft_ns, row_nr, 0, {
...
vim.api.nvim_set_decoration_provider(ns, {
	...
	on_line = function(_, win_id, buf_nr, row_nr)
		if not is_cursor_in_row(win_id, row_nr) then
			return false
		end
		...
		if is_comment(line) then
			vim.api.nvim_buf_add_highlight(buf_nr, draft_ns, "NonText", row_nr, 0, #line)
		else
...
function M.render(buf_nr, tab_length)
	vim.api.nvim_buf_clear_namespace(buf_nr, -1, 0, -1)
	...
	for nr, line in ipairs(lines) do
		...
		if is_comment(line) then
			...
			make_indent(buf_nr, row_nr, tab_length)
			make_quotes(buf_nr, row_nr, line)
			make_dialogues(buf_nr, row_nr, line)
		end
	end
